<%-- 
    Document   : anomalies
    Created on : May 27, 2019, 10:39:33 PM
    Author     : wgv85
--%>
    ...
    <body>
        <div class="container-fluid">
            ...
            <c:if test="${pageContext.request.userPrincipal.name != null}">
                ...
                <h2>Known Anomalies</h2>
                
                <sec:authorize access="hasRole('ROLE_USER')">
                <form action="addAnomalyPage" method="GET">
                    <button style="border:3px solid black" class="btn btn-block" type="submit"> Add A New Anomaly to the Database!</button>
                </form>
                </sec:authorize>
                ...
                <table class="table table-bordered">
                    ...
                    <c:forEach items="${anomaliesList}" var="anomaly">
                        <tr>
                            <td>${anomaly.anomalyName}</td>
                            <td>
                                <form action="detailsAnomalyPage" method="GET">
                                    <button type="submit" name="detailAnomalyId" value="${anomaly.anomalyId}">Show Details</button>
                                </form>
                            </td>
                            <sec:authorize access="hasRole('ROLE_ADMIN')">
                            <td>
                                <form action="updateAnomalyPage" method="GET">
                                    <button type="submit" name="updateAnomalyId" value="${anomaly.anomalyId}">Update</button>
                                </form>
                            </td>
                            </sec:authorize>
                            <sec:authorize access="hasRole('ROLE_ADMIN')">
                            <td>
                                <form action="deleteAnomaly" method="POST">
                                    <button type="submit" name="deleteAnomaly" value="${anomaly.anomalyId}">Delete</button>
                                </form>
                            </td>
                            </sec:authorize>
                        </tr>
